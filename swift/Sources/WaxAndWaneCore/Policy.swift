...
public func clampUnit(_ value: Float) -> Float { min(max(value, 0.0), 1.0) }

public func normalizeAmbient(_ ambient: Float, dark: Float, bright: Float, gamma: Float) -> Float {
    let linear = clampUnit((ambient - dark) / (bright - dark))
    return pow(linear, gamma)
}

public func mapAmbient(_ ambient: Float, minValue: Float, maxValue: Float, invert: Bool) -> Float {
...
public struct RingBuffer {
    ...
    public var mean: Float { count > 0 ? total / Float(count) : 0 }
    public var isEmpty: Bool { count == 0 }
}

public func thresholdForDelta(_ delta: Float, changeThreshold: Float, riseThreshold: Float?, fallThreshold: Float?) -> Float {
    if delta > 0 { return riseThreshold ?? changeThreshold }
    if delta < 0 { return fallThreshold ?? changeThreshold }
    return changeThreshold
}

public func targetForControl(
